...
async function updateUser(req, res) {
    ...
    const newUpdate = await (0, user_service_1.updateUserService)(req.body, userId);
    return res.status(201).json(newUpdate);
}
exports.updateUser = updateUser;
async function deleteUser(req, res) {
    const userId = Number(req.params.userId);
    const delUser = await (0, user_service_1.deleteUserService)(userId);
    if (!delUser) {
...
async function getUser(req, res) {
    try {
        const userId = Number(req.params.userId);
        const user = await (0, user_service_1.getSingleUserService)(userId);

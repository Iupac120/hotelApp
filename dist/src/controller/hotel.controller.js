...
async function updateHotel(req, res) {
    ...
    const newUpdate = await (0, hotel_service_1.updateHotelService)(hotelId, req.body);
    return res.status(201).json(newUpdate);
}
exports.updateHotel = updateHotel;
async function deleteHotel(req, res) {
    const hotelId = Number(req.params.hotelId);
    const deleHotel = await (0, hotel_service_1.deleteHotelService)(hotelId);
...
async function getHotel(req, res) {
    try {
        ...
        const hotel = await (0, hotel_service_1.getSingleHotelService)(hotelId);

...
const jwt_utils_js_2 = require("../utils/jwt.utils.js");
...
async function reIssueAccessToken(refreshToken, req) {
    ...
    const decoded = (0, jwt_utils_js_1.verifyJwt)(refreshToken);
    console.log("decoded", decoded);
    if (!decoded)
        throw new error_handler_js_1.UnAuthorizedError("Token does not exist");
    const { id } = decoded.decoded;
    console.log("id", id);
    const user = await connect_js_1.default.query(auth_queries_js_1.findUserSession, [id]);
    if (!user)
        throw new error_handler_js_1.UnAuthorizedError("User does not exist");
    console.log("user", user.rows[0]);
    const newuser = user.rows[0];
    const userAgent = req.get("user-agent") || "";
    const accesToken = (0, jwt_utils_js_2.signJwt)({ ...newuser, id: newuser.user_id, isAdmin: newuser.is_admin, userAgent }, {

...
function createUserResetPassword(input, userId, token) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const salt = yield bcrypt_1.default.genSalt(config_1.default.get("saltWorkFactor"));
        const newPassword = yield bcrypt_1.default.hash(input.password, salt);
        const updatePassword = yield connect_1.default.query(user_queries_1.updateUserPassword, [newPassword, userExist.email]);
        return user.rows[0].email;
    });
}
exports.createUserResetPassword = createUserResetPassword;
function verifyUserOtp(input, otpEmail) {
    return __awaiter(this, void 0, void 0, function* () {
        const userExist = yield connect_1.default.query(user_queries_1.getUserByEmail, [otpEmail]);
        ...
        const verifyUser = yield connect_1.default.query(user_queries_1.updateUserIsVerify, [otpEmail]);
        return verifyUser.rows[0];
    });
}
exports.verifyUserOtp = verifyUserOtp;
function resendVerifyUserOtp(input) {
    return __awaiter(this, void 0, void 0, function* () {
        const userExist = yield connect_1.default.query(user_queries_1.getUserByEmail, [input]);
        if (!userExist.rows.length)
        ...
        const deleteOtp = yield connect_1.default.query(user_queries_1.updateTokenByQuery, [input]);
        if (deleteOtp) {
            const salt = yield bcrypt_1.default.genSalt(config_1.default.get('saltWorkFactor'));
            const otp = yield (0, random_utils_1.generateOtp)();
            const otpDate = Date.now() + 1800000;
            const otpTime = new Date(otpDate);
            const hashedOtp = yield bcrypt_1.default.hash(otp, salt);
            const updateOtp = yield connect_1.default.query(user_queries_1.updateUserResetToken, [hashedOtp, otpTime, input]);
            //E-mail message
            const message = `
